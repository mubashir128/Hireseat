...
export class VideoCallComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.publishedStreamSubscription = this.opentokService._publishedStream.subscribe(
      (publishedStream) => {
        this.spinner.show();
  ...
  ngOnInit() {
    ...
    //
    this.opentokService.initSessionAPI(this.candidateId).subscribe(
      (session: OT.Session) => {
        this.spinner.show();
  ...
  getPersonalInfo(payload) {
    ...
    this.videoCallingService.getPersonalInfoById(payload).subscribe(
      (res) => {
        this.candidateInfo = res.isCandidate;
        // this.opentokService.setCandidateId(this.candidateId);
        // console.log(this.candidateInfo);
  ...
  getSelectedCandidateInfo(payload) {
    ...
    this.videoCallingService.getCandidatesInfoById(payload).subscribe(
      (res) => {
        this.candidateInfo = res.isCandidate;
        this.opentokService.setCandidateId(this.candidateId);
        // console.log(this.candidateInfo);
  ...
  startArchive() {
    this.startArchiveSubscription = this.videoCallingService
      .startArchive({ sessionId: this.session.sessionId })
      .subscribe(
        (res) => {
          if (res) {
  ...
  storeArchive = (): integer => {
    ...
    // add subscription
    this.videoCallingService.storeArchive(payload).subscribe(
      (res) => {
        // console.log(res);
        if (res) {
          this.responseArchivedId = res.archiveId;
          this.emailConfirmPopup(
            "The archived link is added to the candidate's resume",
            3000
          );
          this.viewArchiveButton = false;
  ...
  submitReview() {
    ...
    // console.log(payload);
    this.videoCallingService.submitRecruitersReview(payload).subscribe(
      (res) => {
        if (res) {
          // console.log(res);
