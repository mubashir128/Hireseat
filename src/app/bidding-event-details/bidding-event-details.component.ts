...
export class BiddingEventDetailsComponent implements OnInit {
  ...
  Base64: any;
    ...
    id: string;
  constructor(private userService:UserService,private router: Router, private route: ActivatedRoute,private _AuthService:AuthenticationService,private spinner:NgxSpinnerService,
    private bidEventService:BiddingEventService,private feedbackService: FeedbackService) {
    let a = this;
    this.Base64 = {
        // private property
        ...
        encode: function (input) {
            ...
            while (i < input.length) {
                ...
                output = output +
                    a.Base64._keyStr.charAt(enc1) + a.Base64._keyStr.charAt(enc2) +
                    a.Base64._keyStr.charAt(enc3) + a.Base64._keyStr.charAt(enc4);
  ...
  handleRequest(_id){
    this.spinner.show();
    this.bidEventService.getBiddingEventById(_id).subscribe((data:any)=>{
...
configureMuut() {
  ...
  var signature = this.SHA1('Vi13o9HvxxN1mjhh78lW8gBb' + ' ' + message + ' ' + timestamp);
