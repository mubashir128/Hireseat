...
export class NewResumeComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,private resumeService:ResumeService,private userService:UserService,private router:Router,private spinner:NgxSpinnerService) { 
  ...
  ngOnInit() {    
    ...
    this.newResumeFrm = this.formBuilder.group({
      ...
      socialSecurityNum:['', Validators.compose([Validators.required,Validators.pattern('[0-9]+'),Validators.maxLength(4)])]     
      });
  ...
  submit(){ 
    ...
    if(this.newResumeFrm.valid && this.resume.fileURL != ""){
      this.resumeService.createResume(this.resume).subscribe((data:any)=>{             
