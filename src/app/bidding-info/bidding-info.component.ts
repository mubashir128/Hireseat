...
export class BiddingInfoComponent
  ...
  get frm() {
    return this.resumePoints.controls;
  }
  ngAfterViewInit() {
  ...
  handleRequest(key) {
    this.feedbackService.getBidsById(key).subscribe(
      (data) => {
        if (data != null) {
  ...
  feedBack(resumeBidId, interviewOrReject, commentId, index) {
    ...
    if (
      ...
      this.ratingPoints.skillMatch = 50;
      this.ratingPoints.experienceMatch = 50;
      this.ratingPoints.educationMatch = 50;
      this.ratingPoints.rejectOrInterview = 100;
      this.ratingPoints.total =
        this.ratingPoints.skillMatch +
        this.ratingPoints.experienceMatch +
        this.ratingPoints.educationMatch +
  ...
  populateBids() {
    this.spinner.show();
    this.bidService.getAllBids(this.biddingEvent.$key).subscribe(
      (data: IBid[]) => {
        this.bids = data;
        if (this.bids.length) {
  ...
  getBiddingDetails() {
    this.spinner.show();
    this.bidService.getBiddingDetails(this.biddingEvent.$key).subscribe(
      (data: any) => {
  ...
  populateMyBids() {
    this.spinner.show();
    this.bidService
      .getRecruiterBids(this.loggedUser._id, this.biddingEvent.$key)
      .subscribe(
        (data: any) => {
          this.spinner.hide();
          this.myBids = data;
  ...
  populateResumes() {
    ...
    this.resumeService.getAllResume().subscribe(
      (data: IResume[]) => {
        this.resumes = data;
        if (this.resumes.length > 0) this.isShow = true;
  ...
  submitBid() {
    if (this.bidFrm.valid) {
      ...
      if (
        this.myBids.findIndex(
          (obj) => obj.resumeKey._id == this.bid.resumeKey
        ) == -1
      ) {
  ...
  seeVideo(i, archiveId, resume) {
    ...
    this.seeArchivedVideoSubscription = this.videoCallingService
