...
export class EditTagComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.tagForm
      .get("tag")
      .valueChanges.subscribe(res => console.log("tag component", res));
  }
  createTagForms() {
  ...
  addTag(tag) {
    ...
    //   return;
    // }
    this.blogService.addNewTag(tag).subscribe(
      response => {
    ...
    console.log("ready output", this.tagList);
  ...
  removeTag(tag) {
    ...
    //   return;
    // }
    this.blogService.removeNewTag(tag).subscribe(
      response => {
        if (response.result == "success") {
